...
<div class="container continut panou">
    <section>
        <?php if ( isset( $_SESSION['id'] ) && $utilizator_logat['status'] == 'Admin' ) { ?>
        <article>
            <h2 class="titlu-site">Modifica testimonial</h2>

            <form action="panou.php#testimonails" method="post">
                <input type="hidden" name="id" value="<?php echo $id ?>"/>
                <fieldset>
                    <label for="nume">Testimonial: </label>
                    <textarea name="nume" id="nume"><?php echo $testimonial['testimonial']; ?></textarea>
                </fieldset>

                <fieldset>
                    <label for="autor">Autorul testimonialului: </label>
                    <input type="text" name="autor" value="<?php echo $testimonial["autor_testimonial"]; ?>"/>
                </fieldset>

                <fieldset>
                    <button type="submit" name="modifica-testimonial">Modifica</button>
                </fieldset>
            </form>
        </article>
        <?php } else { ?>
