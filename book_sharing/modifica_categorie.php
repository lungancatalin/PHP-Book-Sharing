...
$categoriile = mysqli_query($con, $sql);
$categoria_curenta = mysqli_fetch_assoc($categoriile);
...
<div class="container continut panou">
    <section>
        <?php if ( isset( $_SESSION['id'] ) && $utilizator_logat['status'] == 'Admin' ) { ?>
        <article>
            <h2 class="titlu-site">Modifica categoria cartii</h2>
            ...
            <form action="panou.php#categorii" method="post">
                ...
                <fieldset>
                    ...
                    <input type="text" name="nume" value="<?php echo $categoria_curenta['nume_categorie']; ?>"/>
                </fieldset>

                <fieldset>
                    <button type="submit" name="modifica-categorie">Modifica</button>
                </fieldset>
            </form>
        </article>
        <?php } else { ?>
